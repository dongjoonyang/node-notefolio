...
function fnInfinityScroll(json) {
    $(document).on("scroll", function(){
        ...
        if($(window).scrollTop() + $(window).height() == $(document).height()) {    
            console.log("Project Scroll : ok");

            if(subId == undefined) {
                if(json.length !== -1) {
                    let off = json.off + 5;
                    console.log("Project main off :" + off);
                    console.log("Project main length :" + json.length);
                    $.ajax({
                        type : "get",
                        url : "/main/" + mainId + "/off/" + off,
                        dataType : "JSON",
                ...
                if(json.length !== -1) {
                    let off = json.off + 5;
                    console.log("Project sub off :" + off);
                    console.log("Project sub length :" + json.length);
                    $.ajax({
                        type : "get",
